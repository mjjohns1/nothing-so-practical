...
The [first post](/post/causal-inference-is-easy/) in this series argued that causal inference boils down to eliminating rival explanations. The [second](/post/causal-inference-is-hard/) showed why that's hard in practice. Along the way, we introduced DAGs and the backdoor criterion as tools for figuring out what to control for. We even wrote down the adjustment formula with its $\text{do}(X)$ notation and moved on without unpacking it.

That notation deserves a closer look. The $\text{do}$ operator sits at the heart of Pearl's causal framework. The do-calculus built around it is what makes DAGs useful beyond visualization. It gives you a set of rules for determining when a causal question can be answered with observational data, and if so, what to compute. The rules cover situations the backdoor criterion can't handle, and they work on any DAG.

All of which sounds abstract. So let's ground it in something concrete.

Suppose a university wants to know whether its recommended SAT prep course actually improves students' scores. They can't run an experiment. Students self-select into the course, and the kind of student who signs up for test prep isn't a random draw from the population. The university has observational data on about 5,000 students. The naive comparison shows that students who took the prep course scored 140 points higher on average. But nobody should believe that number without asking what else might explain it.
...
In notation, the gap is $P(\text{SAT} \mid \text{Prep} = 1) - P(\text{SAT} \mid \text{Prep} = 0)$, a comparison conditioned on what we *observe*. The causal question needs something different: $P(\text{SAT} \mid \text{do}(\text{Prep} = 1))$. The $\text{do}$ means we're not looking at who chose prep. We're asking what would happen if we *assigned* it, overriding all the reasons students normally select into the course.
...
{{< figure src="/img/posts/do-calculus/sat-dag-do.svg" caption="The mutilated graph under do(Prep). Arrows into Prep Course have been severed (gray dashed), breaking every backdoor path. The highlighted node is now set externally rather than determined by confounders." class="img-center" >}}
...
The formal notation captures exactly when each simplification is valid, stated in terms of conditional independence in modified versions of the graph. You don't need to memorize the notation to understand the ideas, but it's there when you need to verify that a particular identification strategy is sound.

The real power of these rules comes from chaining them. Start with a causal quantity you want to estimate (something with a $\text{do}$ in it). Apply the rules repeatedly, simplifying step by step, until every $\text{do}$ is gone and the expression contains only things you can compute from data. If you get there, the causal effect is identified. If you can't eliminate all the $\text{do}$ operators no matter what sequence of rules you try, the effect isn't identifiable from the data you have. The do-calculus doesn't just give you tools. It tells you when those tools aren't enough.
...
Chain the two pieces together. Prep adds 11 hours of study. Each additional hour adds some number of SAT points (estimated from piece two). Multiply them and you have the causal effect of prep on SAT scores, without ever measuring motivation.
...
where $M$ is the mediator (Hours Studied), $X$ is the treatment (Prep Course), and $Y$ is the outcome (SAT Score). The first sum captures the effect of treatment on the mediator. The second captures the effect of the mediator on the outcome, adjusted for treatment to block the backdoor.
