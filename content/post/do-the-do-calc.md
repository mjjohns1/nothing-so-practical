...
The do-operator sits at the heart of Pearl's causal inference framework. It's kind of odd at first glance. The $\text{do}()$ notation seems like jargon that disguises something fairly easy to understand. You draw a DAG (directed acyclic graph), find the backdoor paths, and adjust for those variables. Isn't this just an obtuse way of saying "control for confounders"?

For simple problems, the do-operator can be seen as shorthand for eliminating all confounding. But it can be useful when confounders can't be measured. Do-calculus can tell you if a causal effect is still identifiable in the data. Whether that matters in practice depends on how much you trust your DAG. The goal of this post is to demistify the do-operator using a concrete example to show what it does and when it can be useful.
...
The arrows encode how background factors feed into both the decision to take a prep course and SAT performance directly.

In notation, the naive comparison can be expressed:

$E[\text{SAT} \mid \text{Prep} = 1] - E[\text{SAT} \mid \text{Prep} = 0]$.

This conditions on who *chose* to take a prep course, and by doing so, it also conditions on all the factors that influenced that choice. These are the backdoor paths, and they inflate the estimate.
...
This formula says: Calculate the treatment effect among people with the same confounder values ($Z = z$), then average across all confounder values weighted by how common they are in the sample. $Z$ must block all backdoor paths from $X$ to $Y$ and must not include anything caused by the treatment.
...
Epidemiologists will recognize the adjustment formula as stratification by another name. You can also accomplish backdoor adjustment by adding the confounders to a regression of Y on X.
...
The $\text{do}$-operator is the notation that connects all three. The critical insight is that $P(Y \mid \text{do}(X))$ is NOT the same as $P(Y \mid X)$. When we calculate the average SAT score of students who took a prep course, $P(\text{SAT} \mid \text{Prep} = 1)$, the people contributing to that value tend to be from wealthier households, be more academically motivated, and have higher GPAs, because those are the people who chose to do prep. The average is tilted toward students who would have scored well regardless. That's why the naive estimate is 138 points.

$P(\text{SAT} \mid \text{do}(\text{Prep} = 1))$ asks what the average SAT score would be if students were assigned to take a prep course, regardless of background. The adjustment formula accomplishes this by reweighting. Instead of letting the confounder distribution of prep-takers drive the average, it weights each confounder stratum by its share of the overall sample. That's what the $P(Z=z)$ term does. If you adjust for every confounder, the estimate will drop from 138 to something close to the true effect of 55.

### When the Backdoor Fails

Up to this point, the do-operator hasn't really told us much beyond the backdoor criterion alone. Do-calculus becomes useful when the backdoor criterion fails. To fully block the backdoor paths, we need to adjust for academic motivation. But motivation isn't in our dataset. Because it affects both whether a student takes a prep course and how well they score, there's a backdoor path we can't block.

But we do have data on how many hours each student studied per week. Prep courses increase study time, and study time improves scores. If the entire causal effect of prep on SAT scores flows through hours studied, which is a *very* strong assumption, we have a front-door path.
...
**Piece one: SAT Prep Course --> Hours Studied.** Academic motivation makes some students more likely to take prep, and motivation also makes them study more. This DAG assumes motivation only reaches hours studied through taking a prep course. The observed relationship between prep and hours is therefore unconfounded. Students who take the course study about 11 more hours on average. No adjustment needed.

**Piece two: Hours Studied --> SAT Score.** This one is trickier. Motivation affects SAT scores directly, and it reaches hours studied indirectly through Prep. That opens a backdoor path between hours studied and SAT score: Hours ← Prep ← Motivation → SAT. To close that path we need to control for prep course. Among students who all took prep (or didn't), the variation in hours studied is no longer driven by the choice to take prep. Controlling for Prep holds the selection process constant, so within each group the relationship between hours and SAT scores reflects studying time, not differences in who chose to enroll.

<!-- {{< do-calculus-viz >}} -->

Prep adds about 11 hours of study time. Each additional hour adds almost 5 SAT points (estimated from piece two). Multiply them and we get 54 points. Far below the naive 138-point gap, and right near the true effect of 55. All without ever measuring motivation.[^1]
...
where $M$ is the mediator (Hours Studied), $X$ is the treatment (Prep Course), and $Y$ is the outcome (SAT Score). The first sum over $M$,represents the effect of the treatment on the mediator. The second sum over $X$ captures the effect of the mediator on the outcome, controlling for the treatment.

{{% /notation-box %}}

#### Front-Door in Practice

The front-door formula reduces to two regression models.
...
**Step 2.** Regress SAT Score on Hours Studied, controlling for Prep to block the backdoor path:
...
This front-door estimate relies on the strong assumption that motivation affects study hours *only* through the decision to take prep. This is unlikely valid. Motivated students are going to study more regardless of whether they take a prep course. The point is to illustrate the method, not to defend this particular DAG. However, it's worth noting that the front-door adjustment is rarely used in practice because the required conditions are rarely found in real-world data.

### Where the Rules Come In

The front-door formula emerges from the three rules that make up do-calculus. The rules are simple if you think of a DAG as a network of pipes. Association flows through them like water. Every arrow (edge) is a pipe, every node is a junction, and an intervention is a valve that shuts off all incoming pipes to a node. Graph surgery using the do-operator is the process of closing valves.

Each rule answers a simple question about the plumbing:

| | Heuristic | What it checks |
|:---|:---|:---|
| **Rule 1** | **Irrelevant gauge** | This variable isn't connected to the outcome. Reading the gauge tells you nothing new. Ignore it. |
